...
while true
    ...
    try
        ...
        M = csvread(dir2, 14, 0);   %% read parametersscaleBTarget
        scaleBNeon(i) = M(1,7);
    catch ME
        break
    end
...
hold on
for i = 1:4
    plot(scaleBNeon,wallHit(i,:),plotHelp(i));
end
grid on
xlabel('$\textrm{scale}B_{\textrm{Neon}}$')
ylabel('$\textrm{No hits}$')
xlim([0.8 3.1])
legend('$\textrm{First wall}$','$\textrm{Second wall}$','$\textrm{Third wall}$','$\textrm{Fourth wall}$','Location','East')
titleHelp = {"$\textbf{Number of hits for different scaling of neon coils magnetic}$", "$\textbf{ field for scaleBTarget = 2 and scaleBDipole = 1.2}$"};
title(titleHelp)
