...
function plot_scaleBTargetHits()
...
dirStart1 = "\\wsl.localhost\Ubuntu\home\lars\Geant4\Task3\build\targetSecondWall";
dirStart2 = "\\wsl.localhost\Ubuntu\home\lars\Geant4\Task3\build\targetSecondWallParameters";
...
hold on
for i = 1:4
    plot(scaleBTarget,wallHit(i,:),plotHelp(i));
end
grid on
xlabel('$\textrm{scale}B_{\textrm{Target}}$')
ylabel('$\textrm{No hits}$')
xlim([0.8 3.1])
legend('$\textrm{First wall}$','$\textrm{Second wall}$','$\textrm{Third wall}$','$\textrm{Fourth wall}$','Location','East')
title('$\textbf{Number of hits for different scaling of target coil magnetic field}$')
