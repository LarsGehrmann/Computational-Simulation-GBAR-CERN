...
void testRatioTarDi(ConstructionParameters* constructionParameters, RunParameters* runParameters, G4RunManager* runMan, double TarStart,
    ...
    for (int TarRun = 0; TarRun < noTar; ++TarRun) { // for every fixed target scaling
        ...
        for (int DiRun = 0; DiRun < noDi; ++DiRun) {
            ...
            fileNameWalls = "hitsScalingTarDiRunWalls" + std::to_string(TarRun) + "DiRun" + std::to_string(DiRun) + ".csv";
            fileNameParameters = "hitsScalingTarDiRunParameters" + std::to_string(TarRun) + "DiRun" + std::to_string(DiRun) + ".csv";
